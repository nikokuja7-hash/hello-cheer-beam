  ...
  async function loadPayouts() {
    try {
      const { data, error } = await supabase
        .from("payouts")
        .select("id,user_id,amount_kes,phone,status,created_at,marked_paid_at,tournament_id,profiles:user_id(username),tournaments:tournament_id(name)")
  ...
  async function markAsPaid(payout: Payout) {
    try {
      ...
      await supabase.from("notifications").insert({
        ...
        title: "Prize Paid!",
        body: `Your prize of KES ${payout.amount_kes.toLocaleString()} has been sent to your M-Pesa`,
        link: "/profile",
      });
  ...
  if (loading) {
    return (
      <div className="min-h-dvh bg-background pb-24">
        <div className="mx-auto max-w-md px-5 py-10 text-center text-muted-foreground">Loading payouts...</div>
        <BottomNav />
      </div>
    );
  }
    ...
    <div className="min-h-dvh bg-background pb-24">
      <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur-xl">
        <div className="mx-auto max-w-md px-5 py-4">
          <h1 className="font-display text-xl tracking-wider flex items-center gap-2">
            <DollarSign className="h-5 w-5 text-primary" /> Payouts
          </h1>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-md px-5 py-6 space-y-6">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Total Owed</p>
        ...
        <div className="space-y-2">
          ...
          {payouts.length === 0 ? (
            ...
            <div className="space-y-2">
              {payouts.map((payout) => (
                <div key={payout.id} className={`rounded-lg border p-3 ${payout.status === "pending" ? "border-yellow-500/40 bg-yellow-500/5" : payout.status === "sent" ? "border-green-500/40 bg-green-500/5" : "border-red-500/40 bg-red-500/5"}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      {payout.phone && <p className="text-[9px] text-muted-foreground">{payout.phone}</p>}
                    </div>
                    <div className="text-right">
                      <p className="font-display text-lg">KES {payout.amount_kes.toLocaleString()}</p>
                      <Badge variant={payout.status === "sent" ? "default" : payout.status === "pending" ? "secondary" : "destructive"} className="text-[9px] mt-1">{payout.status}</Badge>
                    </div>
                  </div>
                  {payout.status === "pending" && (
                    <Button onClick={() => markAsPaid(payout)} size="sm" className="mt-2 h-7 w-full text-[10px]">
